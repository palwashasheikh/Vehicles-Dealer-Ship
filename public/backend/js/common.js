...
function sendAjaxForm(form) {
    ...
    .then(response => {
        if (response.status == 200) {
            ...
            if (dt !== '') (datatable ?? $(dt).DataTable()).ajax.reload();
            if (redirect) {
...
$(function() {
    ...
    $('.date-range-picker').on('apply.daterangepicker', function(ev, picker) {
        ...
        $(this).val(dateRange).change(); // Set value and trigger change event
    });
    $('.date-range-picker').on('cancel.daterangepicker', function(ev, picker) {
        $(this).val('').change(); // Clear value and trigger change event
    });
});
